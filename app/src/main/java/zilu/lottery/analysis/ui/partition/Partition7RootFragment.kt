...
import zilu.lottery.analysis.ui.rp7.R7PAnalysisFragment
import zilu.lottery.analysis.ui.rp7.R7PTypeContinuationFragment
import zilu.lottery.analysis.ui.rp7.R7PTypeFragment
 ...
 */
class Partition7RootFragment(private val name: String) : BaseFragment(),
    TabLayout.OnTabSelectedListener {

    constructor() : this("前区7分区父Fragment")
    ...
    override fun onCreateView(
        ...
        tabs.setTabItems(R.array.r7p_analysis_tabs)
    ...
    private fun showFragment(index: Int, name: String) {
        ...
        val fragment = manager.findFragmentByTag("r7f$index")
        if (fragment == null) {
            transaction.replace(R.id.partitionRootContainer, getFragment(index, name), "r7f$index")
            transaction.commit()
        }
    }

    private fun getFragment(index: Int, name: String): Fragment = when (index) {
        -1 -> Partition7MainFragment(name)
        0 -> R7PTypeFragment(name)
        1 -> R7PTypeContinuationFragment(name)
        else -> R7PAnalysisFragment(name)
    }
}