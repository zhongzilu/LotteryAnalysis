...
public class MultiLineDrawFormat<T> extends TextDrawFormat<T> {
    ...
    public int measureHeight(Column<T> column, int position, TableConfig config) {
        ...
        String text = column.format(position);
        StaticLayout sl;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            sl = new StaticLayout(text, textPaint, width, Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
        }
        return sl.getHeight();
    }
    ...
    public void draw(Canvas c, Rect rect, CellInfo<T> cellInfo ,TableConfig config) {
        ...
        c.save();
        c.translate(DrawUtils.getTextCenterX(rect.left+hPadding,rect.right-hPadding,textPaint), rect.top+(rect.height()-staticLayout.getHeight())/2f);
        staticLayout.draw(c);
        c.restore();
    }
}
